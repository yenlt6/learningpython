...
SELECT *
FROM products
WHERE unit_price BETWEEN 3 AND 5; unit_price>=3 and unit_price<=5
...
SELECT *
FROM customers
WHERE last_name REGEXP "^\.a";# \. dai dien cho 1 ky tu,  ^: dai dien cho bat dau 1 chuoi trước đó ko có gì
...
SELECT * FROM `customers` order by city DESC limit 5 OFFSET 3
=> sắp xếp theo thứ tự giảm dần của city theo chữ cái alphabet, 
lấy 5 bản ghi đầu tiên kể từ bản ghi index=3 trở đi ( bỏ qua index=0,1,2: bỏ qua 3 bản ghi trên cùng)

SELECT * FROM `customers` order by points asc => Sắp xếp theo điểm từ thấp tới cao,


SELECT * FROM `customers` order by points asc limit 5 OFFSET 5 => Lấy 5 bản ghi, 
bỏ 5 bản ghi có số điểm thấp nhất 
( 5 bản ghi có số điểm thấp nhất này sẽ ở trên cùng và có index = 0,1,2,3,4)
